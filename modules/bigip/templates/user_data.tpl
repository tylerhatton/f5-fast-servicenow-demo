...
extension_packages: 
  install_operations:
    ...
    - extensionType: fast
      ...
      extensionUrl: https://github.com/F5Networks/f5-appsvcs-templates/releases/download/v1.10.0/f5-appsvcs-templates-1.10.0-1.noarch.rpm
extension_services: 
  service_operations:
    - extensionType: do
      type: inline
      value:
        ...
        class: Device
        async: true
        label: 1NIC with Good Licensing
        Common:
          class: Tenant
          dbVars:
            class: DbVariables
...
for i in {1..30}; do
   curl -fv --retry 1 --connect-timeout 5 -L https://github.com/F5Networks/f5-bigip-runtime-init/releases/download/1.2.1/f5-bigip-runtime-init-1.2.1-1.gz.run -o "/var/config/rest/downloads/f5-bigip-runtime-init.gz.run" && break || sleep 10
done
# Install
